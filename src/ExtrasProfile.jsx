...
const ExtrasProfile = () => {
  ...
  const editar = () => {
    axios.post('http://localhost/extras_update.php', form)
      .then(response => {
  ...
  const eliminar = (id) => {
    ...
    if (opcion) {
      axios.post('http://localhost/extras_delete.php', { id })
        .then(response => {
          if (response.data.success) {
  ...
  const insertar = () => {
    if (form.nombre && form.precio && form.image_url) {
      axios.post('http://localhost/extras_create.php', form)
        .then(response => {
          if (response.data.success) {
